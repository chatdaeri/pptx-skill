...
| **1. bg-image / pseudo 장식 자동 캡처 마킹** | 모든 element 에 대해 `getComputedStyle(el).backgroundImage !== 'none'` 또는 `getComputedStyle(el, ':before'/':after')` 의 content + 시각 스타일(bg/border) 이 있으면 `data-pptx-image="true"` 부여. 부모가 이미 마킹돼있으면 스킵. | 사진 배경, gradient 오버레이, eyebrow 옆 데코 라인 등 누락 |
...
각 phase 는 모두 `el.closest('[data-pptx-image]')` 체크해서 캡처 영역 안은 건드리지 않는다 (PNG 라스터라 어차피 DOM 무관).

전체 구현 예시는 `~/contents-studio/test-deck/wrap-text.cjs` (있으면) 참고. 없으면 위 표 기준으로 한 파일 새로 작성. `node_modules` 가 없는 곳에서 돌리려면 SKILL_DIR 의 `bootstrap.cjs` 를 require 해서 playwright 자동 해결.
...
const TARGET_RE = /^에스코어|^SCDream|^S-?Core/i;
